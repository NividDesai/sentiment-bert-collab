...
def test_tokenize_dataframe_shapes():
...
def test_tokenize_dataframe_max_length():
    """Test tokenization with custom max_length"""
    tokenizer = get_tokenizer()
    df = pd.DataFrame({"text": ["test sentence"], "label": [1]})
    out = tokenize_dataframe(df, tokenizer, max_length=64)
...
def test_train_val_split_no_overlap():
    ...
    df = pd.DataFrame({
        'text': [f'unique_sample_{i}' for i in range(50)],
        'label': [0, 1] * 25
    })
